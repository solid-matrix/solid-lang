/**
 * @file test_util.h
 * @brief Minimal test harness shared by every subproject's unit tests.
 * @author solid-matrix
 * @version 0.0.5
 *
 * Lives in common/ so that all modules link one canonical copy through
 * the propagated include path; it is part of the test surface only and
 * must never be included by production code.
 *
